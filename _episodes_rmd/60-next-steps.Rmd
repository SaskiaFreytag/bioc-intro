...
This is what it looks like. We will explain in more detail below.

```{r SE-show, message = FALSE, echo = FALSE}
se <- readRDS("~/Downloads/se2.rds")
...
Importantly, the samples and features are coordinated. If we need to exclude one sample from the assay, it will be automatically removed from the sample metadata in the same operation. 
...
If we take a look at the sample metadata we can see GSM2545336 is now not there.

```{r}
colData(se_good)
```

If we take a look at the assay information we can see there are now no counts for GSM2545336.
